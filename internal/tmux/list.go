...
func ListAllPanes() ([]PaneInfo, error) {
	// Format: pane_id, session_name, window_index, window_name, pane_index, pane_current_command
	format := "#{pane_id}\t#{session_name}\t#{window_index}\t#{window_name}\t#{pane_index}\t#{pane_current_command}"
...
// GetPaneByID returns PaneInfo for a specific pane ID, or nil if not found.
func GetPaneByID(paneID string) (*PaneInfo, error) {
